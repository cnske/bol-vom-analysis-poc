...
def SMHI_Metops(api_parameter:dict):
    ...
    Args: 
        api_paramter: a dictionary containing the parameters to request
            Example: {
                ...
                'data': 'json' # Will be ovverridden to 'csv' if period is corrected-archive
            }
    Return: 
    ...
    # Automatically enforece CSV if period is corrected-archive
    if api_parameter.get('period') == 'corrected-archive':
        api_parameter['data'] = 'csv'
        print("Notice: 'corrected-archive' requested. Forcing data format to be CSV")
    else:
        api_parameter['data'] = api_parameter.get('data', 'json')
    ...
    for par in api_parameter['parameter']:
        ...
        if api_parameter['data']=='csv':
            station_name = response.text.splitlines()[1].split(';')[0]
            station_id = response.text.splitlines()[1].split(';')[1]
